...
\begin{figure}[ht]
	\centering
	\includegraphics[width=\linewidth]{heatmap_ar.png}
	\caption{Example AR in heatmap}
	\label{fig:ar-heatmap}
\end{figure}
...
\subsection{Florida Man}
\textbf{User Story:} \textit{As a responsive user, I want to select my birthday or any other specific date in a calendar, so that I can view a list of reports that were recorded on that day, making it easier to explore historical sightings connected to (personally) meaningful or interesting dates.}
\\
% Input: JJJJ, Input: MM, Input: TT
\textbf{Justification:} The inputs are collected via three individual input fields, in which the user writes integers and after the user has written down a date, like \textit{YEAR-MONTH-DAY} it jumps to the input date and displays the day inside the calendar heatmap in a different color. In addition it lists all reports of the selected date.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{Florida_man.png}
    \caption{Example of a marked heatmap}
    \label{fig:florida_man}
\end{figure}
